...
public class Utillity {
    ...
    */
    ...
    public static void resetOpNummers(planStep[] currOperators, planStep[] allOperators){
        for(int i=0;i<allOperators.length;i++){
            allOperators[i].setOpNummer(i);
        }
        /*ArrayList<Integer> missingOperators = new ArrayList<Integer>();
        int n=0;
        for(int i=0;i<currOperators.length;i++){
            if(!(currOperators[i].getLabel().equals(allOperators[n].getLabel())&&currOperators[i].getOpNummer()==allOperators[n].getOpNummer())){
                missingOperators.add(n);
               i--;
            }
            n++;
        }
        for(;n<allOperators.length;n++){
            missingOperators.add(n);
        }
        for (int i = 0; i < missingOperators.size(); i++) {
            for(int k=0;k<currOperators.length;k++){
                if(currOperators[k].getOpNummer()>=missingOperators.get(i)){
                    currOperators[k].setOpNummer(currOperators[k].getOpNummer()+1);
                }
            }
        }*/
    }
